...
import { Search, ChevronDown, Play, Filter, Edit, Users, Send, Building, Globe, Briefcase, Paperclip, MoreVertical, Plus, Archive } from 'lucide-react';
  ...
  return (
    <div className="flex-1 p-8 bg-[#F6F8F7] flex flex-col min-h-0 overflow-y-auto">
      <div className="flex items-center gap-3 mb-6">
        <Briefcase className="w-8 h-8 text-[#10201C]" />
        <h1 className="text-2xl font-bold text-[#10201C]">Leads List <span className="font-medium">/ {list.name}</span></h1>
      </div>

      {/* 4 Cards at Top */}
      <div className="grid grid-cols-4 gap-4 mb-6">
        {/* Card 1: Email Verified */}
        <div className="bg-[#F6F8F7] border border-[#D3DEDB] rounded-lg p-4 flex flex-col justify-center">
          <div className="flex justify-between items-center mb-2">
            <span className="font-bold text-[#10201C]">Email Verified</span>
            <span className="font-bold text-[#445751]">71%</span>
          </div>
          <div className="w-full bg-[#D9D9D9] h-2 rounded-full overflow-hidden">
            <div className="bg-gradient-to-r from-[#0D8C7C] to-[#8000FF] h-full" style={{ width: '71%' }} />
          </div>
        </div>
        ...
        <div className="bg-[#F6F8F7] border border-[#D3DEDB] rounded-lg p-4 flex flex-col justify-center">
          <div className="flex justify-between items-center mb-2">
            <span className="font-bold text-[#10201C]">Score Fit</span>
            <span className="font-bold text-[#445751]">71</span>
          </div>
          <p className="text-[13px] font-bold text-[#7C8C87] mb-1">Strong Fit</p>
          <div className="w-full bg-[#D9D9D9] h-2 rounded-full overflow-hidden">
            <div className="bg-[#0D8C7C] h-full" style={{ width: '71%' }} />
          </div>
        </div>

        {/* Card 3: Health */}
        <div className="bg-[#F6F8F7] border border-[#D3DEDB] rounded-lg p-4 flex flex-col justify-center">
          <span className="font-bold text-[#10201C] mb-2">Health</span>
          <div className="w-full bg-[#D9D9D9] h-2 rounded-full flex overflow-hidden mb-2">
            <div className="bg-[#00C11A]" style={{ width: '57%' }} />
            <div className="bg-[#FF823A]" style={{ width: '29%' }} />
            <div className="bg-[#E20000]" style={{ width: '14%' }} />
          </div>
          <div className="flex gap-4">
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full bg-[#00C11A]" />
              <span className="text-[10px] text-[#C3C3C3] font-mono">57%</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full bg-[#FF823A]" />
              <span className="text-[10px] text-[#C3C3C3] font-mono">29%</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full bg-[#E20000]" />
              <span className="text-[10px] text-[#C3C3C3] font-mono">14%</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#F6F8F7] border border-[#D3DEDB] rounded-lg p-4 flex flex-col justify-center">
          <div className="flex justify-between items-center mb-2">
            <span className="font-bold text-[#10201C]">Active Campaigns</span>
            <span className="font-bold text-[#445751]">3</span>
          </div>
          <p className="text-[13px] font-bold text-[#7C8C87] mb-1">Total Enrolled</p>
          <div className="w-full bg-[#D9D9D9] h-2 rounded-full overflow-hidden">
            <div className="bg-[#8000FF] h-full" style={{ width: '100%' }} />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="relative w-[300px]">
          <Search className="absolute left-3 top-2.5 w-5 h-5 text-[#7C8C87]" />
          <input 
            type="text" 
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="flex items-center gap-2 px-4 py-2 text-[#E20000] bg-white border border-[#D3DEDB] rounded-lg shadow-sm hover:bg-gray-50 ml-2">
          ...
          <button className="flex items-center gap-2 px-4 py-2 bg-[#0D8C7C] text-white rounded-lg shadow-sm hover:bg-[#14B39F]">
            <span className="text-lg leading-none mb-0.5">+</span> New Leads
          </button>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-[#ECF6F5] sticky top-0 z-10 border-b border-[#D3DEDB]">
              <tr>
                <th className="p-4 w-12">
                  <input type="checkbox" checked={selectedIds.size === listLeads.length && listLeads.length > 0} onChange={toggleAll} className="w-4 h-4 rounded border-gray-300 accent-[#0D8C7C]" />
                </th>
            ...
            <tbody>
              {listLeads.map((lead) => {
                ...
                const healthType = Number(lead.id) % 3 === 0 ? 'Risky' : Number(lead.id) % 5 === 0 ? 'Invalid' : 'Verified';

                return (
                  <tr key={lead.id} className={`border-b border-[#D3DEDB] transition-colors ${isSelected ? 'bg-[#0D8C7C]/5' : 'hover:bg-black/5'}`}>
                    <td className="p-4">
                      <input type="checkbox" checked={isSelected} onChange={() => toggleOne(lead.id)} className="w-4 h-4 rounded border-gray-300 accent-[#0D8C7C]" />
                    </td>
                    <td 
                    ...
                    <td className="p-4">
                      <span className={cn(
                        ...
                        healthType === 'Verified' ? "bg-[#00C11A]/10 text-[#00B218]" :
                        healthType === 'Risky' ? "bg-[#FF823A]/10 text-[#FF823A]" :
                        "bg-[#E20000]/10 text-[#E20000]"
                      )}>
                        {healthType}
                      </span>
                    </td>
                    <td className="p-4 font-bold text-[#0D8C7C]">{lead.fitScore}%</td>
                    <td className="p-4">
              ...
              {listLeads.length === 0 && (
                <tr>
                  <td colSpan={9} className="p-8 text-center text-[#7C8C87]">No leads found in this list.</td>
                </tr>
              )}
            </tbody>
          </table>
