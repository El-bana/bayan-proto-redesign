'use client';
import { useAppStore, LeadList } from '@/lib/store';
import { ChevronDown, Play, Filter, Archive, Edit, Users, Search, MoreVertical, Paperclip, Send } from 'lucide-react';
          ...
          <table className="w-full text-left">
            <thead className="bg-[#ECF6F5] sticky top-0 z-10 border-b border-[#D3DEDB]">
              <tr>
                <th className="p-4 w-12">
                  <input type="checkbox" checked={selectedIds.size === lists.length && lists.length > 0} onChange={toggleAll} className="w-4 h-4 rounded border-gray-300 accent-[#0D8C7C]" />
                </th>
            ...
            <tbody>
              {lists.map((list) => {
                ...
                return (
                  <tr key={list.id} className={`border-b border-[#D3DEDB] transition-colors ${isSelected ? 'bg-[#0D8C7C]/5' : 'hover:bg-black/5'}`}>
                    <td className="p-4">
                      <input type="checkbox" checked={isSelected} onChange={() => toggleOne(list.id)} className="w-4 h-4 rounded border-gray-300 accent-[#0D8C7C]" />
                    </td>
                    <td 
                    ...
                    <td className="p-4 text-[#10201C] font-medium">{list.leadIds.length}</td>
                    <td className="p-4 text-[#10201C] font-medium">American ICP</td>
                    ...
                    <td className="p-4">
                      <span className="bg-[#00C11A]/20 text-[#00B218] px-2 py-1 rounded-md text-xs font-medium">Fresh</span>
                    </td>
                    <td className="p-4 text-[#10201C] font-medium">3 Days ago</td>
                    <td className="p-4">
